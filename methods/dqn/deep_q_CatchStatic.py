...
class DeepQNetwork:
    def __init__(self, name, env):
        ...
        self.max_reward = -100
        self.save_path = os.getcwd() + '\\methods\\dqn\\models\\weights\\DQN_CatchStatic_' + name + '.h5f'
        self.load_path = os.getcwd() + '\\methods\\dqn\\models\\chks\\temp_interval_10_episodes_198_.h5f'
    ...
    def build_callbacks(self):
        checkpoint_weights_filename = os.getcwd() + '\\methods\\dqn\\models\\chks\\chkDQN_CatchStatic_' + self.name + '_{step}.h5f'
        log_filename = os.getcwd() + '\\methods\\dqn\\chks\\weights\\LOG_DQN_CatchStatic_{}_log.json'.format(self.name)
        callbacks = [ModelIntervalCheckpoint(checkpoint_weights_filename, interval=1000)]
        tel = TrainIntervalLogger(model_name='CatchStatic')
        callbacks += [tel]
    ...
    def run(self):
        ...
        history = self.dqn.fit(self.env, nb_steps=self.nb_steps, visualize=True, verbose=1, nb_max_episode_steps=self.max_steps, log_interval=self.log_interval, callbacks=[TrainIntervalLogger(model_name='CatchStatic')])
        print(history)
